...
fn test_detect_secrets_multiple_secrets() {
    ...
    let text = "ABCD CUSTOM_ghp_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX foo baz warp-server-staging.firebaseapp.com bar \n foo CUSTOM_sk_live_XXXXXXXXXXXXXXXXXXXXXXXX qux foo";
    let detected_secrets = find_secrets_in_text(text);
    assert_eq!(
        detected_secrets,
        vec![
...
fn test_detect_secrets_default_regex_case_sensitivity() {
    ...
    let text = "API keys: CUSTOM_sk_test_XXXXXXXXXXXXXXXXXXXXXXXX CUSTOM_SK_TEST_XXXXXXXXXXXXXXXXXXXXXXXX";
    let detected_secrets = find_secrets_in_text(text);
    assert_eq!(
        detected_secrets,
        vec![SecretRange {
