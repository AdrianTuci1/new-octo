...
function buildTabConfigTabId(fileName: string) {
...
export function useAppWindow() {
  ...
  const [paneStartupCommandsByPaneId, setPaneStartupCommandsByPaneId] = useState<Record<string, string[]>>({});
  ...
  const handleOpenTabConfig = useCallback(async (configPath: string) => {
    try {
      ...
      const readyCloudProfile = getDefaultReadyCloudProfile(memorySettings?.values);
      ...
      const plan = buildTabConfigLaunchPlan(parsed, {
        tabId,
        homeDir: pathContext?.homeDir ?? null,
        cloudTarget,
        resolvedVariables
      });
  ...
  }, [memorySettings?.values, pathContext?.homeDir]);
